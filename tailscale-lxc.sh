...
if [[ ${#CTID_MENU[@]} -eq 0 ]]; then
  msg_error "No LXC containers found on this node"
  exit 1
fi
...
while [[ -z "${CTID}" ]]; do
  CTID=$(whiptail --backtitle "Proxmox VE Helper Scripts" --title "Containers on $NODE" --radiolist \
    "\nSelect a container to add Tailscale to:\nUse SPACE to select, TAB to navigate, ENTER to confirm\n" \
    18 $((MSG_MAX_LENGTH + 23)) 6 \
  ...
  # If CTID is still empty, show error and continue loop
  if [[ -z "${CTID}" ]]; then
    whiptail --backtitle "Proxmox VE Helper Scripts" --title "Selection Required" --msgbox \
      "Please select a container from the list.\n\nInstructions:\n• Use ARROW keys to navigate\n• Press SPACE to select/deselect\n• Press ENTER to confirm selection" 10 55
  fi
done
